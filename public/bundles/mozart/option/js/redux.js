...
(function ( $ ) {
	...
	$.redux.initEvents = function () {
		...
		$( '#toplevel_page_' + redux.params.slug + ' .wp-submenu a, #wp-admin-bar-' + redux.params.slug + ' a.ab-item' ).click(
			function ( e ) {
				if ( $( '#toplevel_page_' + redux.params.slug ).hasClass( 'wp-menu-open' ) || $( this ).hasClass( 'ab-item' ) ) {
					e.preventDefault();
		...
		$( 'input[name="' + redux.params.opt_name + '[defaults]"]' ).click(
			function () {
				if ( !confirm( redux.params.reset_confirm ) ) {
					return false;
				}
				window.onbeforeunload = null;
			}
		...
		// Default button clicked
		$( 'input[name="' + redux.params.opt_name + '[defaults-section]"]' ).click(
			function () {
				if ( !confirm( redux.params.reset_section_confirm ) ) {
					return false;
				}
	...
	$.redux.initQtip = function () {
		if ( $().qtip ) {
			// Shadow
			var shadow = '';
			var tip_shadow = redux.params.hints.tip_style.shadow;
			...
			// Color
			var color = '';
			var tip_color = redux.params.hints.tip_style.color;
			...
			// Rounded
			var rounded = '';
			var tip_rounded = redux.params.hints.tip_style.rounded;
			...
			// Tip style
			var style = '';
			var tip_style = redux.params.hints.tip_style.style;
			...
			// Get position data
			var myPos = redux.params.hints.tip_position.my;
			var atPos = redux.params.hints.tip_position.at;
			...
			var showEvent = redux.params.hints.tip_effect.show.event;
			var hideEvent = redux.params.hints.tip_effect.hide.event;

			// Tip show effect
			var tipShowEffect = redux.params.hints.tip_effect.show.effect;
			var tipShowDuration = redux.params.hints.tip_effect.show.duration;

			// Tip hide effect
			var tipHideEffect = redux.params.hints.tip_effect.hide.effect;
			var tipHideDuration = redux.params.hints.tip_effect.hide.duration;
	...
	$.redux.tabCheck = function () {
		$( '.redux-group-tab-link-a' ).click(
			function () {
				...
				el.find( '#toplevel_page_' + redux.params.slug + ' .wp-submenu a.current' ).removeClass( 'current' );
				el.find( '#toplevel_page_' + redux.params.slug + ' .wp-submenu li.current' ).removeClass( 'current' );

				el.find( '#toplevel_page_' + redux.params.slug + ' .wp-submenu a' ).each(
					function () {
						var url = $( this ).attr( 'href' ).split( '&tab=' );
				...
				redux.errors.errors, function ( sectionID, sectionArray ) {
					$.each(
						sectionArray.errors, function ( key, value ) {
							$( "#" + redux.params.opt_name + '-' + value.id ).addClass( "redux-field-error" );
							if ( $( "#" + redux.params.opt_name + '-' + value.id ).parent().find( '.redux-th-error' ).length === 0 ) {
								$( "#" + redux.params.opt_name + '-' + value.id ).append( '<div class="redux-th-error">' + value.msg + '</div>' );
							}
						}
					);
				}
	...
	$.redux.required = function () {
		...
		$.each(
			redux.folds, function ( i, v ) {
				var fieldset = $( '#' + redux.params.opt_name + '-' + i );
	...
	$.redux.get_container_value = function ( id ) {
		var value = $( '#' + redux.params.opt_name + '-' + id ).serializeForm();

		if ( value !== null && typeof value === 'object' && value.hasOwnProperty( redux.params.opt_name ) ) {
			//console.log('object');
			//console.log(value);
			value = value[redux.params.opt_name][id];
		}
		//console.log(value);
		return value;
	};
	...
	$.redux.check_dependencies = function ( variable ) {
		...
		$.each(
			redux.required[id], function ( child, dependents ) {
				...
				var current = $( this ),
				    show = false,
				    childFieldset = $( '#' + redux.params.opt_name + '-' + child ),
					...
					tr.fadeIn(
						300, function () {
							...
							if ( redux.required.hasOwnProperty( child ) ) {
								$.redux.check_dependencies( $( '#' + redux.params.opt_name + '-' + child ).children().first() );
							}
							$.redux.initFields();
						}
					);
	...
	$.redux.required_recursive_hide = function ( id ) {
		var toFade = $( '#' + redux.params.opt_name + '-' + id ).parents( 'tr:first' );

		toFade.fadeOut(
			50, function () {
				$( this ).addClass( 'hide' );

				if ( $( '#' + redux.params.opt_name + '-' + id ).hasClass( 'redux-container-section' ) ) {
					var div = $( '#section-' + id );
					if ( div.hasClass( 'redux-section-indent-start' ) ) {
				...
				if ( $( '#' + redux.params.opt_name + '-' + id ).hasClass( 'redux-container-info' ) ) {
					$( '#info-' + id ).fadeOut( 50 ).addClass( 'hide' );
				}

				if ( $( '#' + redux.params.opt_name + '-' + id ).hasClass( 'redux-container-divide' ) ) {
					$( '#divide-' + id ).fadeOut( 50 ).addClass( 'hide' );
				}

				if ( $( '#' + redux.params.opt_name + '-' + id ).hasClass( 'redux-container-raw' ) ) {
					var rawTable = $( '#' + redux.params.opt_name + '-' + id ).parents().find( '.redux-raw-table' );
		...
		if ( redux.required_child.hasOwnProperty( id ) ) {
			$.each(
				redux.required_child[id], function ( i, parentData ) {
					if ( $( '#' + redux.params.opt_name + '-' + parentData.parent ).parents( 'tr:first' ).hasClass( '.hide' ) ) {
						show = false;
					} else {
						if ( show !== false ) {
...
var confirmOnPageExit = function ( e ) {
	...
	var message = redux.params.save_pending;
...
function redux_change( variable ) {
	...
	if ( !redux.params.disable_save_warn ) {
