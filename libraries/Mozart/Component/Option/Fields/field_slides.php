...
if ( !class_exists ( 'ReduxFramework_slides' ) ) {
    ...
    class ReduxFramework_slides
    {
        ...
        public function render()
        {
            ...
            echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'mozart-options' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ($slides as $slide) {
                    ...
                    echo '<span class="button media_upload_button" id="add_' . $x . '">' . __ ( 'Upload', 'mozart-options' ) . '</span>';
                    ...
                    echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide[ 'attachment_id' ] . '">' . __ ( 'Remove', 'mozart-options' ) . '</span>';
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'title' ] ) : __ ( 'Title', 'mozart-options' );
                    ...
                    if ($this->field[ 'show' ][ 'description' ]) {
                        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'description' ] ) : __ ( 'Description', 'mozart-options' );
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'url' ] ) : __ ( 'URL', 'mozart-options' );
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . sprintf ( __ ( 'Delete %s', 'mozart-options' ), $this->field[ 'content_title' ] ) . '</a></li>';
            ...
            if ($x == 0) {
                ...
                echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args[ 'opt_name' ] . '[' . $this->field[ 'id' ] . '][attachment_id]">' . __ ( 'Remove', 'mozart-options' ) . '</span>';
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __ ( 'Add %s', 'mozart-options' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }
        ...
        public function enqueue()
        {
            ...
            wp_enqueue_style (
                ...
                ReduxFramework::$_url . 'src/fields/media/field_media.css',
                time (),
                true
            );
            ...
            wp_enqueue_script (
                ...
                ReduxFramework::$_url . 'src/fields/slides/field_slides' . Redux_Functions::isMin () . '.js',
            ...
            wp_enqueue_style (
                ...
                ReduxFramework::$_url . 'src/fields/slides/field_slides.css',
                time (),
                true
            );
        }
