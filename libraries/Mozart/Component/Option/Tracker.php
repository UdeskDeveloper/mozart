...
class Tracker
{
    ...
    public function load( $builder )
    {
        ...
        if (!isset( $this->options['hash'] ) || !$this->options['hash'] || empty( $this->options['hash'] )) {
            ...
            update_option( 'mozart-options-tracking', $this->options );
        }
        ...
        if (isset( $_GET['page'] ) && $_GET['page'] == $this->builder->args['page_slug']) {
            if (!isset( $this->options['allow_tracking'] )) {
                add_action( 'admin_enqueue_scripts', array( $this, '_enqueue_tracking' ) );
            } elseif (!isset( $this->options['tour'] ) && ( $this->builder->args['dev_mode'] == "true" || $this->builder->args['page_slug'] == "redux_demo" )) {
    ...
    function tracking_request()
    {
        ...
        $content = '<h3>' . __( 'Help improve Our Panel', 'mozart-options' ) . '</h3>';
        $content .= '<p>' . __(
                'Please helps us improve our panel by allowing us to gather anonymous usage stats so we know which configurations, plugins and themes to test to ensure compatability.',
                'mozart-options'
            ) . '</p>';
        $opt_arr = array(
            'content'  => $content,
        ...
        $button2 = __( 'Allow tracking', 'mozart-options' );
        ...
        $this->print_scripts(
            $id,
            $opt_arr,
            __( 'Do not allow tracking', 'mozart-options' ),
            $button2,
            $function2,
            $function1
        );
    ...
    function newsletter_request()
    {
        ...
        $content .= '<p><strong>' . __( 'Getting Started', 'mozart-options' ) . '</strong><br>' . sprintf(
                __(
                    'This panel demonstrates the many features of Redux.  Before digging in, we suggest you get up to speed by reviewing %1$s.',
                    'mozart-options'
                ),
                '<a href="http://docs.reduxframework.com/mozart-options/getting-started/" target="_blank">' . __(
                    'our documentation',
                    'mozart-options'
                ) . '</a>'
            );
        $content .= '<p><strong>' . __( 'Redux Generator', 'mozart-options' ) . '</strong><br>' . sprintf(
                __(
                    'Want to get a head start? Use the %1$s. It will create a customized boilerplate theme or a standalone admin folder complete with all things Redux (with the help of Underscores and TGM). Save yourself a headache and try it today.',
                    'mozart-options'
                ),
                '<a href="http://generate.reduxframework.com/" target="_blank">' . __(
                    'Redux Generator',
                    'mozart-options'
                ) . '</a>'
            );
        $content .= '<p><strong>' . __( 'Redux Extensions', 'mozart-options' ) . '</strong><br>' . sprintf(
                __(
                    'Did you know we have extensions, which greatly enhance the features of Redux?  Visit our %1$s to learn more!',
                    'mozart-options'
                ),
                '<a href="http://reduxframework.com/extensions/" target="_blank">' . __(
                    'extensions directory',
                    'mozart-options'
                ) . '</a>'
            );
        $content .= '<p><strong>' . __( 'Like Redux?', 'mozart-options' ) . '</strong><br>' . sprintf(
                __(
                    'If so, please %1$s and consider making a %2$s to keep development of Redux moving forward.',
                    'mozart-options'
                ),
                '<a target="_blank" href="http://wordpress.org/support/view/plugin-reviews/mozart-options">' . __(
                    'leave us a favorable review on WordPress.org',
                    'mozart-options'
                ) . '</a>',
                '<a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=N5AD7TSH8YA5U" target="_blank">' . __(
                    'donation',
                    'mozart-options'
                ) . '</a>'
            );
        $content .= '<p><strong>' . __( 'Newsletter', 'mozart-options' ) . '</strong><br>' . __(
                'If you\'d like to keep up to with all things Redux, please subscribe to our newsletter',
                'mozart-options'
            ) . ':</p>';
        $content .= '<form action="http://reduxframework.us7.list-manage2.com/subscribe/post?u=564f5178f6cc288064f332efd&amp;id=ace5bbc1f9&SOURCE=panel" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate><p style="text-align: center;"><label for="mce-EMAIL">' . __(
                'Email address',
                'mozart-options'
            ) . ' </label><input type="email" value="" name="EMAIL" class="required email" id="mce-EMAIL"><input type="hidden" value="panel" name="SOURCE">&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="' . __(
                'Subscribe',
                'mozart-options'
            ) . '" name="subscribe" id="mc-embedded-subscribe" class="button button-primary"></p></form>';
        $opt_arr = array(
            'content'      => $content,
            'position'     => array( 'edge' => 'top', 'align' => 'center' ),
            'pointerWidth' => 450
        ...
        $this->print_scripts( $id, $opt_arr, __( 'Close', 'mozart-options' ), false, '', $function1 );
    }
    ...
    function trackingObject()
    {
        ...
        $data = array(
            ...
            'site'      => array(
                ...
                'users'     => $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT COUNT(*) FROM $wpdb->users INNER JOIN $wpdb->usermeta ON ({$wpdb->users}.ID = {$wpdb->usermeta}.user_id) WHERE 1 = 1 AND ( {$wpdb->usermeta}.meta_key = %s )",
...
function redux_allow_tracking_callback()
{
    ...
    if (wp_verify_nonce( $_REQUEST['nonce'], 'redux_activate_tracking' )) {
        $options = get_option( 'mozart-options-tracking' );
        ...
        if (update_option( 'mozart-options-tracking', $options )) {
            die( '1' );
        } else {
            die( '0' );
        }
