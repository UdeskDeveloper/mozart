    ...
    class acf_field_taxonomy extends acf_field
    {
        ...
        */
        ...
        public function ajax_query()
        {
            ...
            $args = apply_filters(
                'acf/fields/taxonomy/query/name=' . $field['name'],
                $args,
                $field,
                $options['post_id']
            );
            $args = apply_filters(
                'acf/fields/taxonomy/query/key=' . $field['key'],
                $args,
                $field,
                $options['post_id']
            );
        ...
        */
        ...
        public function get_term_title( $term, $field, $post_id = 0 )
        {
            ...
            $title = apply_filters(
                'acf/fields/taxonomy/result/name=' . $field['_name'],
                $title,
                $term,
                $field,
                $post_id
            ...
            $title = apply_filters(
                'acf/fields/taxonomy/result/key=' . $field['key'],
                $title,
                $term,
                $field,
                $post_id
                ...
                <?php if ($field['field_type'] == 'radio' && $field['allow_null']): ?>
                    <li>
                        <div class="radio radio-default">
                            <input type="radio" name="<?php echo $field['name']; ?>" id="<?php echo $field['name']; ?>" value=""/>
                            <label for="<?php echo $field['name']; ?>"> <?php _e( "None", 'acf' ); ?>
                            </label>
                        </div>
                    </li>
                <?php endif; ?>
    ...
    class acf_taxonomy_field_walker extends Walker
    {
        ...
        public function start_el( &$output, $term, $depth = 0, $args = array(), $current_object_id = 0 )
        {
            ...
            if ($this->field['field_type'] == 'checkbox') {
                ...
                $output .= '<li><div class="checkbox check-default"><input type="checkbox" id="' . $this->field['name'] . '" name="' . $this->field['name'] . $term->term_id . '" value="' . $term->term_id . '" ' . ( $selected ? 'checked="checked"' : '' ) . ' /><label for="' . $this->field['name'] . $term->term_id . '">' . $term->name . '</label>';

            } elseif ($this->field['field_type'] == 'radio') {

                $output .= '<li><div class="radio radio-default"><input type="radio" id="' . $this->field['name'] . '" name="' . $this->field['name'] . $term->term_id . '" value="' . $term->term_id . '" ' . ( $selected ? 'checked="checkbox"' : '' ) . ' /><label for="' . $this->field['name'] . $term->term_id . '">' . $term->name . '</label>';
