...
function wpb_getImageBySize(
	$params = array( 'post_id' => null, 'attach_id' => null, 'thumb_size' => 'thumbnail', 'class' => '' )
) {
	...
	if (is_string( $thumb_size ) && ( ( !empty( $_wp_additional_image_sizes[$thumb_size] ) && is_array(
					$_wp_additional_image_sizes[$thumb_size]
				) ) || in_array( $thumb_size, array( 'thumbnail', 'thumb', 'medium', 'large', 'full' ) ) )
	) {
		...
		$thumbnail = wp_get_attachment_image(
			...
			$thumb_size,
			false,
			array( 'class' => $thumb_class . 'attachment-' . $thumb_size )
		);
		// TODO: APPLY FILTER
	} elseif ($attach_id) {
...
if (!function_exists( 'siteAttachedImages' )) {
	function siteAttachedImages($att_ids = array())
	{
		...
		$media_images = $wpdb->get_results(
			"SELECT * FROM $wpdb->posts WHERE post_type = 'attachment' order by ID desc"
		);
...
if (!function_exists( 'wpb_debug' )) {
	function wpb_debug()
	{
		if (isset( $_GET['wpb_debug'] ) && $_GET['wpb_debug'] == 'wpb_debug') {
			return true;
		} else {
			return false;
		}
...
function vc_convert_shortcode($m)
{
	...
	if (preg_match(
			'/first/',
			$el_position
		) || empty( $shortcode_attr['width'] ) || $shortcode_attr['width'] === '1/1'
	) {
		$result = '[vc_row]';
	}
	if ($tag != 'vc_column') {
	...
	if ($tag == 'vc_column') {
		$result .= "[{$m_one}{$tag} {$attr_string}]" . preg_replace_callback(
				"/{$pattern}/s",
				'vc_convert_inner_shortcode',
				$content
			) . "[/{$tag}{$m_four}]";
	} elseif ($tag == 'vc_tabs' || $tag == 'vc_accordion' || $tag == 'vc_tour') {
		$result .= "[{$m_one}{$tag} {$attr_string}]" . preg_replace_callback(
				"/{$pattern}/s",
				'vc_convert_tab_inner_shortcode',
				$content
			) . "[/{$tag}{$m_four}]";
	} else {
...
function vc_parse_options_string($string, $tag, $param)
{
	...
	foreach (preg_split( '/\|/', $string ) as $value) {
		if (preg_match( '/\:/', $value )) {
			...
			$option_name = $split[0];
			$option_settings = $option_settings_list[$option_name] = vc_param_options_get_settings(
				$option_name,
				$settings['options']
			);
	...
	if (isset( $settings['options'] )) {
		foreach ($settings['options'] as $setting_option) {
			if ($setting_option['type'] !== 'separator' && isset( $setting_option['value'] ) && empty( $options[$setting_option['name']] )) {
				$options[$setting_option['name']] = $setting_option['type'] === 'checkbox' ? preg_split(
					'/\,/',
